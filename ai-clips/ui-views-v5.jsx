...
function LP_TruckDetail({ localT = 0 }) {
  ...
  const stopRows = [
    { stop: 1, order: 'ORD-2026-0057', from: '1234 Distribution Pkwy, Los Angeles, CA, 90040', to: '456 Airport Rd, Rancho Cucamonga, CA, 91801',          weight: '1,950 lb',  hu: 13, ship: '—', arrival: '4/28/2026', distance: '48 mi' },
    { stop: 2, order: 'ORD-2026-0096', from: '1234 Distribution Pkwy, Los Angeles, CA, 90040', to: '330 Business Park Rd, San Diego, CA, 92169',           weight: '13,600 lb', hu: 17, ship: '—', arrival: '4/29/2026', distance: '172 mi', alt: true },
    { stop: 3, order: 'ORD-2026-0061', from: '1234 Distribution Pkwy, Los Angeles, CA, 90040', to: '2856 Commerce Park Blvd Unit 9, Las Vegas, NV, 89193', weight: '24,000 lb', hu: 16, ship: '—', arrival: '5/2/2026',  distance: '332 mi' },
    { stop: 4, order: 'ORD-2026-0085', from: '1234 Distribution Pkwy, Los Angeles, CA, 90040', to: '3400 Commerce Dr, Tucson, AZ, 85701',                  weight: '180 lb',    hu: 6,  ship: '—', arrival: '5/4/2026',  distance: '410 mi', alt: true },
    { stop: 5, order: 'ORD-2026-0033', from: '1234 Distribution Pkwy, Los Angeles, CA, 90040', to: '2826 W Roosevelt St, Phoenix, AZ, 85009',              weight: '5,270 lb',  hu: 10, ship: '—', arrival: '5/5/2026',  distance: '218 mi' },
  ];
  return (
  ...
  return (
    <V3ViewWrap>
      ...
      <div style={{ position: 'relative', flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        <QuoteShipBehind savedDefaults={savedDefaults} showAccessorials/>
...
function QSField({ label, value = '', placeholder = '', icon = null, dropdown = false, info = false, trailing = null, readOnly = false, flex = 1 }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex }}>
      <div style={{
        flex: 1, position: 'relative', minWidth: 0,
        border: '1px solid #B9C0C9', borderRadius: 4,
        padding: '19px 14px 13px',
        background: readOnly ? '#F1F3F5' : '#fff',
        display: 'flex', alignItems: 'center', gap: 8,
      }}>
        <div style={{
          position: 'absolute', top: -8, left: 11, padding: '0 5px', background: readOnly ? '#F1F3F5' : '#fff',
          fontSize: 12, color: '#6B7280', whiteSpace: 'nowrap',
        }}>{label}</div>
        <div style={{ flex: 1, fontSize: 14.5, color: value ? '#1F2937' : '#9AA1AA', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{value || placeholder}</div>
        {icon === 'search' && <svg width="17" height="17" viewBox="0 0 16 16" fill="none"><circle cx="7" cy="7" r="4.6" stroke={FP.blue} strokeWidth="1.5"/><path d="M10.6 10.6L14 14" stroke={FP.blue} strokeWidth="1.5" strokeLinecap="round"/></svg>}
        {icon === 'map' && <svg width="18" height="18" viewBox="0 0 18 18" fill="none"><path d="M2 4.5l4.5-2 5 2 4.5-2v11l-4.5 2-5-2-4.5 2v-11z" stroke={FP.blue} strokeWidth="1.4" strokeLinejoin="round"/><path d="M6.5 2.5v11 M11.5 4.5v11" stroke={FP.blue} strokeWidth="1.4"/></svg>}
        {dropdown && <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M3 4.5l3 3.5 3-3.5" stroke="#6B7280" strokeWidth="1.5" fill="none" strokeLinecap="round" strokeLinejoin="round"/></svg>}
        {trailing}
        {info && <QSInfo/>}
      </div>
    </div>
...
function QSInfo() {
  return (
    <div style={{
      width: 17, height: 17, borderRadius: 9, border: '1.3px solid #7A8290', color: '#7A8290',
      display: 'flex', alignItems: 'center', justifyContent: 'center',
      fontSize: 11, fontWeight: 700, fontFamily: 'serif', flexShrink: 0,
    }}>i</div>
  );
}

function QSPanel({ title, right, children, bodyPad = '20px 18px' }) {
  return (
    <div style={{ background: '#fff', border: '1px solid #DFE3E8', borderRadius: 3 }}>
      <div style={{
        display: 'flex', alignItems: 'center', gap: 14,
        padding: '15px 18px', background: '#EFEFEF', borderBottom: '1px solid #DFE3E8',
      }}>
        <div style={{ fontSize: 17.5, fontWeight: 600, color: '#1F2937', whiteSpace: 'nowrap' }}>{title}</div>
        <div style={{ flex: 1 }}/>
        {right}
      </div>
      <div style={{ padding: bodyPad, display: 'flex', flexDirection: 'column', gap: 16 }}>{children}</div>
    </div>
  );
}

function QSOutlineBtn({ children, big = false }) {
  return (
    <div style={{
      padding: big ? '15px 20px' : '13px 17px', borderRadius: 4,
...
// Quote/Ship page — mirrors the live product screen.
function QuoteShipBehind({ savedDefaults, showAccessorials = false }) {
  const clockIcon = <svg width="18" height="18" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="6" stroke="#6B7280" strokeWidth="1.3"/><path d="M8 4.5V8l2.5 1.5" stroke="#6B7280" strokeWidth="1.3" strokeLinecap="round"/></svg>;
  const calIcon = <svg width="18" height="18" viewBox="0 0 16 16" fill="none"><rect x="2" y="3" width="12" height="11" rx="1" stroke="#6B7280" strokeWidth="1.3"/><path d="M2 6.4h12 M5 2v2 M11 2v2" stroke="#6B7280" strokeWidth="1.3" strokeLinecap="round"/></svg>;

  return (
    <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column', minHeight: 0, background: '#E9EFF6' }}>
      {/* Page header */}
      <div style={{ background: '#fff', padding: '18px 22px 0', flexShrink: 0 }}>
        <div style={{ fontSize: 25, fontWeight: 700, color: '#0B1117', fontFamily: fpFont.heading, letterSpacing: '-0.015em' }}>Quote/Ship</div>
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 24, marginTop: 10 }}>
          <div style={{ display: 'flex' }}>
            {['GENERAL', 'PRODUCT DETAILS', 'ADDITIONAL DETAILS', 'DOCUMENTS'].map((tab) => {
              ...
              return (
                <div key={tab} style={{
                  padding: '12px 18px 14px', fontSize: 14.5, fontWeight: 700, letterSpacing: '0.03em',
                  color: active ? FP.blue : '#6B7280',
                  borderBottom: active ? `3px solid ${FP.blue}` : '3px solid transparent',
                  fontFamily: fpFont.heading, whiteSpace: 'nowrap',
                }}>{tab}</div>
              );
            })}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, paddingBottom: 12 }}>
            <div style={{
              width: 310, position: 'relative', display: 'flex', alignItems: 'center', gap: 12,
              background: '#fff', border: `2px solid ${FP.blue}`, borderRadius: 4, padding: '17px 14px',
            }}>
              <div style={{ position: 'absolute', top: -9, left: 11, padding: '0 5px', background: '#fff', fontSize: 12, color: FP.blue }}>Order Import</div>
              <svg width="19" height="19" viewBox="0 0 20 20" fill="none"><path d="M10 13a3 3 0 100-6 3 3 0 000 6z" stroke={FP.blue} strokeWidth="1.4"/><path d="M10 1v2 M10 17v2 M1 10h2 M17 10h2 M3.5 3.5l1.5 1.5 M15 15l1.5 1.5 M3.5 16.5l1.5-1.5 M15 5l1.5-1.5" stroke={FP.blue} strokeWidth="1.4" strokeLinecap="round"/></svg>
              <div style={{ width: 1, height: 18, background: '#1F2937' }}/>
              <div style={{ flex: 1, fontSize: 14.5, color: '#9AA1AA', whiteSpace: 'nowrap', overflow: 'hidden' }}>Use a comma to separate</div>
              <svg width="18" height="18" viewBox="0 0 16 16" fill="none"><circle cx="7" cy="7" r="4.6" stroke="#4B5563" strokeWidth="1.5"/><path d="M10.6 10.6L14 14" stroke="#4B5563" strokeWidth="1.5" strokeLinecap="round"/></svg>
            </div>
            {['START CONSOLIDATION', 'RECENT QUOTES', 'TEMPLATES'].map((t) => (
              <QSOutlineBtn key={t} big>{t}</QSOutlineBtn>
            ))}
            <div style={{ width: 170 }}>
              <QSField label="Process Date" value="08/11/2026" trailing={calIcon}/>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Body */}
      <div style={{ flex: 1, minHeight: 0, overflow: 'hidden', position: 'relative', padding: '18px 22px 0' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 22, alignItems: 'start' }}>
          {/* LEFT */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <QSPanel title="Locations" right={
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 40, height: 21, borderRadius: 11, background: '#C5CDD7', position: 'relative' }}>
                  <div style={{ position: 'absolute', top: 2, left: 2, width: 17, height: 17, borderRadius: 9, background: '#fff', boxShadow: '0 1px 2px rgba(0,0,0,0.25)' }}/>
                </div>
                <div style={{ fontSize: 16, color: '#1F2937', whiteSpace: 'nowrap' }}>Search by Zip</div>
              </div>
            }>
              <div style={{ display: 'flex', alignItems: 'stretch', gap: 12 }}>
                <QSField label="Ship From" value="FreightPOP, 1 Rancho Circle, Lake Forest, CA, 9263…" trailing={
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <svg width="17" height="17" viewBox="0 0 16 16" fill="none"><circle cx="7" cy="7" r="4.6" stroke={FP.blue} strokeWidth="1.5"/><path d="M10.6 10.6L14 14" stroke={FP.blue} strokeWidth="1.5" strokeLinecap="round"/></svg>
                    <svg width="18" height="18" viewBox="0 0 18 18" fill="none"><path d="M2 4.5l4.5-2 5 2 4.5-2v11l-4.5 2-5-2-4.5 2v-11z" stroke={FP.blue} strokeWidth="1.4" strokeLinejoin="round"/><path d="M6.5 2.5v11 M11.5 4.5v11" stroke={FP.blue} strokeWidth="1.4"/></svg>
                  </div>
                }/>
                <QSOutlineBtn big>
                  <svg width="15" height="15" viewBox="0 0 14 14" fill="none"><path d="M2 11.6l.6-2.2 6-6 1.9 1.9-6 6-2.5.3z" stroke={FP.blue} strokeWidth="1.3" fill="none" strokeLinejoin="round"/></svg>
                  FORM
                </QSOutlineBtn>
                <QSOutlineBtn big>ADDRESS BOOK</QSOutlineBtn>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 9, color: FP.blue, fontSize: 15.5, fontWeight: 700, letterSpacing: '0.03em', fontFamily: fpFont.heading }}>
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M5 2v12 M5 2L2.5 4.6 M5 2l2.5 2.6 M11 14V2 M11 14l2.5-2.6 M11 14l-2.5-2.6" stroke={FP.blue} strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/></svg>
                <span style={{ whiteSpace: 'nowrap' }}>SWAP FROM &amp; TO</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'stretch', gap: 12 }}>
                <QSField label="Ship To" placeholder="Find Location" icon="search"/>
                <QSOutlineBtn big>
                  <svg width="15" height="15" viewBox="0 0 14 14" fill="none"><path d="M2 11.6l.6-2.2 6-6 1.9 1.9-6 6-2.5.3z" stroke={FP.blue} strokeWidth="1.3" fill="none" strokeLinejoin="round"/></svg>
                  FORM
                </QSOutlineBtn>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: FP.blue, fontSize: 15.5, fontWeight: 700, letterSpacing: '0.03em', fontFamily: fpFont.heading }}>
                <svg width="17" height="17" viewBox="0 0 16 16" fill="none"><path d="M2.5 8.5l3.5 3.5 7.5-8.5" stroke={FP.blue} strokeWidth="1.9" fill="none" strokeLinecap="round" strokeLinejoin="round"/></svg>
                <span style={{ whiteSpace: 'nowrap' }}>VERIFY ADDRESSES</span>
              </div>
            </QSPanel>

            <QSPanel title="Shipment Details" bodyPad="0" right={
              <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                <QSOutlineBtn big><span style={{ fontSize: 18, lineHeight: 1, fontWeight: 400 }}>+</span> ADD OUTER HANDLING UNIT</QSOutlineBtn>
                <svg width="22" height="22" viewBox="0 0 20 20" fill="none"><path d="M10 13a3 3 0 100-6 3 3 0 000 6z" stroke={FP.blue} strokeWidth="1.4"/><path d="M10 1v2 M10 17v2 M1 10h2 M17 10h2 M3.5 3.5l1.5 1.5 M15 15l1.5 1.5 M3.5 16.5l1.5-1.5 M15 5l1.5-1.5" stroke={FP.blue} strokeWidth="1.4" strokeLinecap="round"/></svg>
              </div>
            }>
              <div style={{ display: 'flex', alignItems: 'center', gap: 18, padding: '18px 20px', borderBottom: '1px solid #E5E8ED' }}>
                <svg width="18" height="18" viewBox="0 0 16 16" fill="none"><path d="M4 10l4-4 4 4" stroke="#4B5563" strokeWidth="1.6" fill="none" strokeLinecap="round"/></svg>
                <svg width="14" height="18" viewBox="0 0 12 18" fill="none">{[0,1,2].map(r => [0,1].map(c => <circle key={`${r}${c}`} cx={2 + c * 6} cy={3 + r * 6} r="1.6" fill="#8A9099"/>))}</svg>
                <div>
                  <div style={{ fontSize: 16, color: '#0B1117', display: 'flex', gap: 12 }}>
                    <span style={{ fontWeight: 500 }}>0</span>
                    <span style={{ color: '#C5CDD7' }}>|</span>
                    <span style={{ fontWeight: 500 }}>0x0x0 @ 0 lb</span>
                  </div>
                  <div style={{ fontSize: 13, color: '#6B7280', marginTop: 3 }}>Generic Product</div>
                </div>
                <div style={{ flex: 1 }}/>
                <div style={{ color: FP.blue, fontSize: 14, fontWeight: 700, letterSpacing: '0.03em', fontFamily: fpFont.heading }}>CUBISCAN</div>
                <div style={{ color: '#4B5563', fontSize: 18, lineHeight: 1 }}>⋯</div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 16, padding: '24px 20px 22px' }}>
                <QSField label="Quantity" value="0" trailing={<svg width="18" height="18" viewBox="0 0 16 16" fill="none"><rect x="2.5" y="2" width="11" height="12" rx="1" stroke="#6B7280" strokeWidth="1.2"/><path d="M5 5.5h6 M5 8h6 M5 10.5h4" stroke="#6B7280" strokeWidth="1.2" strokeLinecap="round"/></svg>}/>
                <QSField label="Type" icon="search"/>
                <QSField label="Container Type" dropdown info/>
                <QSOutlineBtn big>INNER PIECES</QSOutlineBtn>
                <QSField label="Length" trailing={<span style={{ fontSize: 14, color: '#6B7280' }}>in</span>}/>
                <QSField label="Width" trailing={<span style={{ fontSize: 14, color: '#6B7280' }}>in</span>}/>
                <QSField label="Height" trailing={<span style={{ fontSize: 14, color: '#6B7280' }}>in</span>}/>
                <QSField label="Weight (per piece)" trailing={<span style={{ fontSize: 14, color: '#6B7280' }}>lb</span>}/>
                <QSField label="Freight Class" dropdown info/>
                <QSField label="NMFC" trailing={<svg width="18" height="18" viewBox="0 0 16 16" fill="none"><rect x="2.5" y="2" width="11" height="12" rx="1" stroke="#6B7280" strokeWidth="1.2"/><path d="M5 5.5h6 M5 8h6 M5 10.5h4" stroke="#6B7280" strokeWidth="1.2" strokeLinecap="round"/></svg>}/>
                <QSField label="Density" readOnly/>
                <QSField label="Cubic Ft" readOnly/>
              </div>
            </QSPanel>
          </div>
          ...
          {/* RIGHT */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <QSPanel title="Schedule Pickup" right={
              <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                <svg width="20" height="20" viewBox="0 0 16 16" fill="none"><path d="M4 10l4-4 4 4" stroke="#4B5563" strokeWidth="1.6" fill="none" strokeLinecap="round"/></svg>
                <QSInfo/>
              </div>
            }>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <QSField label="Pickup Date" placeholder="MM/DD/YYYY" trailing={calIcon}/>
                <QSField label="Delivery Date" placeholder="MM/DD/YYYY" trailing={calIcon}/>
                <QSField label="Ready Time" placeholder="hh:mm aa" trailing={clockIcon}/>
                <QSField label="Open Time" placeholder="hh:mm aa" trailing={clockIcon}/>
                <QSField label="Cutoff Time" placeholder="hh:mm aa" trailing={clockIcon}/>
                <QSField label="Close Time" placeholder="hh:mm aa" trailing={clockIcon}/>
              </div>
              <div style={{ display: 'flex', gap: 16 }}>
                <div style={{ width: 200 }}><QSField label="Transit Days" info readOnly/></div>
                <QSField label="Pickup Request Email" info/>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <QSField label="Dock" icon="search" info/>
                <QSField label="Equipment" value="0 selected" dropdown info/>
              </div>
            </QSPanel>

            {showAccessorials && (
              <QSPanel title="Favorite Accessorials">
            ...
            <QSPanel title="Carrier">
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <QSField label="Carrier" value="Rate Shop" icon="search"/>
                <QSField label="Service" icon="search"/>
                <QSField label="Payee" value="Sender" dropdown/>
                <QSField label="Payment Type" value="Prepaid (Carrier Account Selected Will Be Cha"/>
              </div>
            </QSPanel>
          </div>
        </div>
        ...
        {/* page scrollbar */}
        <div style={{ position: 'absolute', right: 4, top: 10, bottom: 0, width: 9, borderRadius: 5, background: '#DCE2E9' }}>
          <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '46%', borderRadius: 5, background: '#A9B2BC' }}/>
        </div>
      </div>
      ...
      <div style={{
        ...
        background: '#fff', borderTop: '1px solid #DFE3E8',
      }}>
        <div style={{ flex: 1 }}/>
        {['Total Outer Handling Units: 0', 'Total Handling Units: 0', 'Total Weight: 0 LB', 'Total Linear Feet: 0'].map((t) => (
          <div key={t} style={{ fontSize: 15, color: '#1F2937', whiteSpace: 'nowrap' }}>{t}</div>
        ))}
        <div style={{ width: 8 }}/>
        <QSOutlineBtn big><span style={{ fontSize: 16, lineHeight: 1 }}>⋯</span></QSOutlineBtn>
        <QSOutlineBtn big>CLEAR SHIPMENT</QSOutlineBtn>
        <QSOutlineBtn big>PREVIEW BOL</QSOutlineBtn>
        <QSOutlineBtn big>SAVE SHIPMENT</QSOutlineBtn>
        <QSOutlineBtn big>SPOT QUOTE</QSOutlineBtn>
        <div style={{
          padding: '15px 24px', borderRadius: 4, background: '#1565C0', color: '#fff',
          fontSize: 14, fontWeight: 700, letterSpacing: '0.02em', fontFamily: fpFont.heading, whiteSpace: 'nowrap',
        }}>RATE SHOP</div>
      </div>
    </div>
  );
...
function FakePanel({ title, right, children }) {
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ fontSize: 15, fontWeight: 600, color: FP.textDark, fontFamily: fpFont.heading, letterSpacing: '-0.005em', whiteSpace: 'nowrap' }}>{title}</div>
        <div style={{ flex: 1 }}/>
        {right}
      </div>
...
function FakeToggleRow({ label, on = false, highlight = false }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <div style={{ fontSize: 13, color: FP.textDark, fontWeight: highlight ? 600 : 400, whiteSpace: 'nowrap' }}>{label}</div>
    </div>
  );
}
...
function ViewRateShop({ localT = 0 }) {
  const shopping = localT > 8.0 && localT < 9.0;
  const ratesShown = localT >= 9.0;
  const picked = localT > 10.0;
  const booking = localT > 14.5 && localT < 16.2;
  const booked = localT > 16.2;
  const rates = [
  ...
  const overlayOp = v3Clamp((localT - 8.0) / 0.4);
  const cols = '26px 1.5fr 1.1fr 96px 120px 120px 104px';

  return (
    <V3ViewWrap>
      <div style={{ position: 'relative', flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, overflow: 'hidden' }}>
        <QuoteShipBehind savedDefaults={false}/>

        {overlayOp > 0 && (
          <div style={{
            position: 'absolute', inset: 0, background: 'rgba(11,17,23,0.42)',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            opacity: overlayOp, padding: '40px 500px 40px 70px',
          }}>
            <div style={{
              width: '100%', maxWidth: 1240, maxHeight: '100%',
              background: '#fff', borderRadius: 5, overflow: 'hidden',
              boxShadow: '0 24px 70px rgba(0,0,0,0.35)',
              display: 'flex', flexDirection: 'column',
              transform: `translateY(${(1 - overlayOp) * 14}px)`,
            }}>
              {/* Modal header */}
              <div style={{
                display: 'flex', alignItems: 'center', gap: 14,
                padding: '16px 22px', borderBottom: `1px solid ${FP.borderLight}`,
              }}>
                <div style={{ fontSize: 17, fontWeight: 700, color: FP.textDark, fontFamily: fpFont.heading }}>Rate Results</div>
                <div style={{ width: 1, height: 20, background: FP.borderLight }}/>
                <div style={{ fontSize: 12.5, color: FP.subtle }}>Lane: <strong style={{ color: FP.textDark }}>LAX → PHX</strong> · 22,400 lb · LTL</div>
                <div style={{ flex: 1 }}/>
                <div style={{ color: FP.subtle, fontSize: 18, lineHeight: 1 }}>×</div>
              </div>

              {/* Active rules bar */}
              <div style={{
                display: 'flex', alignItems: 'center', gap: 10,
                padding: '10px 22px', background: '#F3F7FC', borderBottom: `1px solid ${FP.borderLight}`,
              }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: FP.blue, fontFamily: fpFont.mono, letterSpacing: '0.06em' }}>ACTIVE RULES</div>
                <div style={{ width: 1, height: 18, background: FP.border }}/>
              ...
              {!ratesShown ? (
                ...
                <div style={{ overflow: 'hidden' }}>
                  <div style={{
                    ...
                    padding: '11px 22px', borderBottom: `2px solid ${FP.borderLight}`, gap: 12, background: '#FAFBFC',
                  }}>
                    {['', 'Carrier', 'Service', 'Transit', 'Performance', 'Total', ''].map((h, i) => (
                      <div key={i} style={{ fontSize: 11.5, color: FP.subtle, fontWeight: 700, letterSpacing: '0.04em' }}>{h}</div>
                    ))}
                  </div>
                  {rates.map((r, i) => {
                    ...
                    return (
                      <div key={i} style={{
                        display: 'grid', gridTemplateColumns: cols,
                        padding: '11px 22px', gap: 12, alignItems: 'center',
                  ...
                  <div style={{
                    display: 'flex', alignItems: 'center', gap: 10,
                    padding: '11px 22px', borderTop: `1px solid ${FP.borderLight}`, background: '#FAFBFC',
                  }}>
                    <div style={{ fontSize: 12, color: FP.subtle }}>7 carrier options · 2 excluded by rules</div>
                    <div style={{ flex: 1 }}/>
                    <FakeBtn>CLOSE</FakeBtn>
                  </div>
                </div>
              )}
            </div>
      ...
      <div style={{ flex: 1, padding: 14, display: 'flex', justifyContent: 'center', overflow: 'hidden' }}>
        <div style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {[
              { h: 'SHIP FROM', l: ['Lake Forest DC', '1 Rancho Cir', 'Lake Forest, CA 92630'] },
    ...
    <div style={{ padding: '28px 34px 12px', color: '#222', fontFamily: 'Arial, sans-serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr', border: '1px solid #2C2C2C', borderTop: 'none' }}>
        <div style={{ ...ld, borderRight: '1px solid #2C2C2C', padding: '8px 10px' }}>
          Lake Forest DC<br/>1 Rancho Cir<br/>Lake Forest, CA, 92630, US<br/>9497651574
        </div>
    ...
    <div style={{ padding: '30px 34px 16px', color: '#222', fontFamily: 'Arial, sans-serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginBottom: 18 }}>
        <div>
          ...
          <div style={ld}>Lake Forest DC<br/>1 Rancho Cir<br/>Lake Forest, CA, 92630<br/>9497651574, shipping@lakeforestdc.com</div>
        </div>
        <div>
      ...
      <div style={{ border: bd }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', borderBottom: bd }}>
          <div style={{ borderRight: bd, padding: '8px 10px', fontSize: 11.5, lineHeight: 1.5 }}>
            <div style={red}>FROM :</div>Lake Forest DC<br/>1 Rancho Cir<br/>Lake Forest, CA, 92630<br/>9497651574
          </div>
  ...
  return (
    <div style={{ padding: '28px 34px 14px', color: '#222', fontFamily: 'Arial, sans-serif' }}>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={ld}>Lake Forest DC<br/>1 Rancho Cir<br/>Lake Forest, CA, 92630<br/>9497651574</div>
      </div>
      ...
      <div style={{ border: bd, borderTop: 'none' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '0.6fr 2.4fr 1fr 1.4fr' }}>
          ...
          <div style={docCell({ fontSize: 10.5, borderRight: bd })}>Lake Forest DC, 1 Rancho Cir, Lake Forest, CA, 92630, US</div>
      ...
      <div style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 10, flex: 1, overflow: 'hidden' }}>
        ...
        {showNotification && (
          <ThreadMessage
            ...
            time="just now"
            body="Hi O'Neil Storage — your shipment from Lake Forest DC is delayed due to a carrier breakdown. New delivery window: Wed 4–6 PM. Apologies for the inconvenience. Reply here if the new window doesn't work."
            tone="out"
...
function ViewInvoiceAudit({ localT = 0 }) {
  const focusA = localT >= 4.2 && localT < 11.8;   // INV-7842 — missed accessorial
  const focusB = localT >= 12.1 && localT < 17.0;  // INV-7840 — detention dispute
  const ruleSaved = localT > 10.5;
  const bulkApproved = localT > 17.3;

  const rows = [
    { carrier: 'Echo Global - FP Echo', scac: 'ECHO', vendor: '', up: '06/10/2026', appr: ruleSaved ? '06/10/2026' : '', audit: '06/10/2026', inv: '06/09/2026', ship: '06/08/2026', hot: focusA },
    { carrier: 'Echo Global - FP Echo', scac: 'ECHO', vendor: '', up: '06/10/2026', appr: '', audit: '06/10/2026', inv: '06/07/2026', ship: '06/05/2026', hot: focusB },
    { carrier: 'AAA Cooper - AAA Cooper', scac: 'AACT', vendor: '', up: '06/09/2026', appr: '', audit: '06/09/2026', inv: '06/09/2026', ship: '06/09/2026' },
    { carrier: 'Cross Country - FP Cross Country', scac: 'CCYQ', vendor: '', up: '06/08/2026', appr: '', audit: '06/08/2026', inv: '04/22/2026', ship: '' },
    { carrier: 'Doug TL - Doug TL', scac: 'DTLC', vendor: '', up: '05/01/2026', appr: '', audit: '05/01/2026', inv: '05/01/2026', ship: '04/22/2026' },
    { carrier: "Garrett's Truckin' - G FTL", scac: '', vendor: '', up: '04/24/2026', appr: '', audit: '04/24/2026', inv: '04/24/2026', ship: '02/21/2026' },
    { carrier: 'A Duie Pyle - FP ADP', scac: 'PYLE', vendor: '', up: '04/17/2026', appr: '', audit: '', inv: '08/01/2025', ship: '' },
    { carrier: 'A Duie Pyle - FP ADP', scac: 'PYLE', vendor: '', up: '04/17/2026', appr: '', audit: '', inv: '07/25/2025', ship: '' },
    { carrier: 'Acertus - ACRT', scac: 'MTGS', vendor: '', up: '04/16/2026', appr: '', audit: '', inv: '', ship: '' },
    { carrier: "Southern Freight - SFWY", scac: '', vendor: '', up: '06/07/2025', appr: '', audit: '', inv: '06/06/2025', ship: '' },
    { carrier: 'Reddaway - RDWY', scac: '', vendor: '', up: '03/14/2025', appr: '', audit: '03/14/2025', inv: '03/14/2025', ship: '03/15/2025' },
    { carrier: 'My Fleet - My Fleet', scac: '', vendor: '', up: '03/10/2025', appr: '', audit: '03/10/2025', inv: '03/10/2025', ship: '02/24/2025' },
    { carrier: 'Amanda Pooling Test - Amanda Pooling', scac: '', vendor: '', up: '03/06/2025', appr: '', audit: '03/06/2025', inv: '03/06/2025', ship: '03/07/2025' },
  ].map((r, i) => (bulkApproved && i > 1 && !r.appr) ? { ...r, appr: r.up } : r);

  const gcols = '46px 300px 200px 200px 200px 200px 200px 200px 190px';
  const cellS = { fontSize: 13.5, color: '#1F2937', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' };
  const checkbox = (
    <div style={{ width: 17, height: 17, border: '1.6px solid #6B7280', borderRadius: 2 }}/>
  );

  return (
    <V3ViewWrap>
      {/* Page header */}
      <div style={{ background: '#fff', padding: '20px 22px 22px', display: 'flex', alignItems: 'center', gap: 16, flexShrink: 0 }}>
        <div style={{ fontSize: 25, fontWeight: 700, color: '#0B1117', fontFamily: fpFont.heading, letterSpacing: '-0.015em', whiteSpace: 'nowrap' }}>Freight Invoice Audit</div>
        <div style={{ flex: 1 }}/>
        <div style={{
          display: 'flex', alignItems: 'center', gap: 9, padding: '13px 18px',
          border: `1px solid ${FP.blue}`, borderRadius: 4, color: FP.blue,
          fontSize: 13.5, fontWeight: 700, letterSpacing: '0.02em', fontFamily: fpFont.heading, whiteSpace: 'nowrap',
        }}>
          <svg width="17" height="17" viewBox="0 0 20 20" fill="none">
            <path d="M8.6 2.4l1.4 3.9 3.9 1.4-3.9 1.4-1.4 3.9-1.4-3.9L3.3 7.7l3.9-1.4 1.4-3.9z" fill={FP.blue}/>
            <path d="M14.8 11.6l.7 1.9 1.9.7-1.9.7-.7 1.9-.7-1.9-1.9-.7 1.9-.7.7-1.9z" fill={FP.blue}/>
          </svg>
          IMPORT WITH AI
        </div>
        <div style={{
          width: 44, height: 44, borderRadius: 22, background: FP.blue, color: '#fff',
          display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 26, lineHeight: 1, fontWeight: 300,
        }}>+</div>
      </div>

      <div style={{ padding: '0 22px 18px', flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
        <div style={{ border: `1px solid ${FP.borderLight}`, background: '#fff', display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
          {/* Invoice Metrics — collapsed band */}
          <div style={{
            display: 'flex', alignItems: 'center', padding: '18px 22px',
            background: '#EDEDED', borderBottom: '1px solid #DDE1E6', flexShrink: 0,
          }}>
            <div style={{ fontSize: 17, fontWeight: 500, color: '#1F2937', fontFamily: fpFont.body, whiteSpace: 'nowrap' }}>Invoice Metrics</div>
            <div style={{ flex: 1 }}/>
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M5 8l5 5 5-5" stroke="#4B5563" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
          </div>

          {/* Toolbar */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 22, padding: '18px 22px', flexShrink: 0 }}>
            <div style={{
              display: 'flex', alignItems: 'center', gap: 12, background: '#fff',
              border: '1px solid #C3C9D2', borderRadius: 4, padding: '13px 16px', width: 300,
            }}>
              <svg width="17" height="17" viewBox="0 0 16 16" fill="none"><circle cx="7" cy="7" r="4.6" stroke="#6B7280" strokeWidth="1.4"/><path d="M10.6 10.6L14 14" stroke="#6B7280" strokeWidth="1.4" strokeLinecap="round"/></svg>
              <div style={{ flex: 1, fontSize: 14.5, color: '#8A9099' }}>Search</div>
              <div style={{ fontSize: 13.5, fontWeight: 700, color: '#4B5563', letterSpacing: '0.04em' }}>GO</div>
            </div>
            <svg width="22" height="22" viewBox="0 0 20 20" fill="none"><path d="M2.5 4h15l-6 6.5V17l-3-1.6v-4.9L2.5 4z" stroke="#6B7280" strokeWidth="1.3" strokeLinejoin="round"/></svg>
            <div style={{ display: 'flex', gap: 12 }}>
              {[
                { t: `${bulkApproved ? 44 : 0} Recently Approved`, on: false },
                { t: '14 Not Approved', on: true },
                { t: 'History', on: false },
              ].map((p) => (
                <div key={p.t} style={{
                  padding: '11px 22px', borderRadius: 999,
                  border: `1px solid ${p.on ? '#0B2545' : '#C3C9D2'}`,
                  background: p.on ? '#0B2545' : '#fff',
                  color: p.on ? '#fff' : '#1F2937',
                  fontSize: 14, fontWeight: p.on ? 600 : 400, whiteSpace: 'nowrap',
                }}>{p.t}</div>
              ))}
            </div>
            <div style={{ flex: 1 }}/>
            <div style={{
              padding: '13px 20px', border: `1px solid ${FP.blue}`, borderRadius: 4,
              color: FP.blue, fontSize: 13.5, fontWeight: 700, letterSpacing: '0.02em',
              fontFamily: fpFont.heading, whiteSpace: 'nowrap',
            }}>EDI INVOICES IMPORT</div>
            <div style={{
              padding: '13px 20px', border: `1px solid ${FP.blue}`, borderRadius: 4,
              color: FP.blue, fontSize: 15, fontWeight: 700, lineHeight: 1,
            }}>⋯</div>
          </div>

          {/* Grid */}
          <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', borderTop: '1px solid #DDE1E6' }}>
            <div style={{
              display: 'grid', gridTemplateColumns: gcols,
              padding: '0 0 0 0', background: '#fff', borderBottom: '1px solid #C9CED6', flexShrink: 0,
            }}>
              {[{ l: '', c: true }, { l: 'Carrier' }, { l: 'Carrier SCAC' }, { l: 'Internal Vendor Id' }, { l: 'Date Uploaded' }, { l: 'Approved Date' }, { l: 'Audit Date' }, { l: 'Invoice Date' }, { l: 'Shipment Date' }].map((h, i) => (
                <div key={i} style={{
                  padding: '14px 16px', fontSize: 14, fontWeight: 600, color: '#1F2937',
                  whiteSpace: 'nowrap', overflow: 'hidden',
                  borderRight: i === 0 ? '1px solid #E3E6EA' : 'none',
                  display: 'flex', alignItems: 'center', justifyContent: i === 0 ? 'center' : 'flex-start',
                }}>{h.c ? checkbox : h.l}</div>
              ))}
            </div>
            ...
            <div style={{ flex: 1, minHeight: 0, overflow: 'hidden', position: 'relative' }}>
              {rows.map((r, i) => (
                <div key={i} style={{
                  display: 'grid', gridTemplateColumns: gcols, alignItems: 'center',
                  borderBottom: '1px solid #E9ECEF',
                  background: r.hot ? '#E8F1FB' : '#fff',
                  boxShadow: r.hot ? `inset 3px 0 0 ${FP.blue}` : 'none',
                  position: 'relative',
                }}>
                  <div style={{ padding: '14px 16px', display: 'flex', justifyContent: 'center', borderRight: '1px solid #E3E6EA' }}>{checkbox}</div>
                  <div style={{ ...cellS, padding: '14px 16px' }}>{r.carrier}</div>
                  <div style={{ ...cellS, padding: '14px 16px' }}>{r.scac}</div>
                  <div style={{ ...cellS, padding: '14px 16px' }}>{r.vendor}</div>
                  <div style={{ ...cellS, padding: '14px 16px' }}>{r.up}</div>
                  <div style={{ ...cellS, padding: '14px 16px', color: r.appr ? FP.green : '#1F2937', fontWeight: r.appr ? 600 : 400 }}>{r.appr}</div>
                  <div style={{ ...cellS, padding: '14px 16px' }}>{r.audit}</div>
                  <div style={{ ...cellS, padding: '14px 16px' }}>{r.inv}</div>
                  <div style={{ ...cellS, padding: '14px 16px' }}>{r.ship}</div>
                  <div style={{
                    position: 'absolute', right: 0, top: 0, bottom: 0,
                    display: 'flex', alignItems: 'center', gap: 14, padding: '0 26px 0 20px',
                    background: r.hot ? '#E8F1FB' : '#fff', borderLeft: '1px solid #E3E6EA',
                  }}>
                    <svg width="18" height="18" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="6.8" fill={FP.blue}/><path d="M5 8.2l2 2 4-4.4" stroke="#fff" strokeWidth="1.5" fill="none" strokeLinecap="round" strokeLinejoin="round"/></svg>
                    <svg width="17" height="17" viewBox="0 0 16 16" fill="none"><path d="M3 12.4l.6-2.4 6.2-6.2 1.8 1.8-6.2 6.2-2.4.6z" stroke={FP.blue} strokeWidth="1.3" fill="none" strokeLinejoin="round"/></svg>
                    <svg width="20" height="17" viewBox="0 0 20 16" fill="none"><rect x="1" y="3" width="10" height="8" rx="1" stroke={FP.blue} strokeWidth="1.3"/><path d="M11 6h4l3 3v2h-7V6z" stroke={FP.blue} strokeWidth="1.3" strokeLinejoin="round"/><circle cx="5" cy="12.6" r="1.5" stroke={FP.blue} strokeWidth="1.2"/><circle cx="14.5" cy="12.6" r="1.5" stroke={FP.blue} strokeWidth="1.2"/></svg>
                    <svg width="17" height="17" viewBox="0 0 16 16" fill="none"><rect x="2.5" y="2" width="11" height="12" rx="1" stroke={FP.blue} strokeWidth="1.3"/><path d="M5 5.5h6 M5 8h6 M5 10.5h4" stroke={FP.blue} strokeWidth="1.2" strokeLinecap="round"/></svg>
                  </div>
                </div>
              ))}
              {/* vertical scrollbar */}
              <div style={{ position: 'absolute', right: 2, top: 6, bottom: 6, width: 8, borderRadius: 4, background: '#EDEFF2' }}>
                <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '38%', borderRadius: 4, background: '#B7BEC7' }}/>
              </div>
            </div>

            {/* horizontal scrollbar rail */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', borderTop: '1px solid #E3E6EA', flexShrink: 0 }}>
              <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M6.5 1.5L3 5l3.5 3.5" stroke="#6B7280" strokeWidth="1.2" strokeLinecap="round"/></svg>
              <div style={{ flex: 1, height: 8, borderRadius: 4, background: '#EDEFF2', position: 'relative' }}>
                <div style={{ position: 'absolute', left: '4%', width: '28%', top: 0, bottom: 0, borderRadius: 4, background: '#B7BEC7' }}/>
              </div>
              <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M3.5 1.5L7 5l-3.5 3.5" stroke="#6B7280" strokeWidth="1.2" strokeLinecap="round"/></svg>
            </div>
          </div>
        </div>
      </div>
