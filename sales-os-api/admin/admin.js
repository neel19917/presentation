...
const S = { token: localStorage.getItem('fp_admin_token') || '', cfg: null, meta: null, defaults: null, dirty: false, section: localStorage.getItem('fp_admin_section') || 'settings', open: {}, revisions: [], variants: null, v: null, vDirty: false, share: { go: '', lock: false, flags: [], tabs: null, filter: '' }, links: null, shortBase: '', linkOpen: null, linkDetail: null, issued: null };
const app = $('#app');
...
function shareCard() {
  ...
  <div class="card"><h3>Send it as a tracked link</h3><p class="hint">A short code that opens the deck exactly as set above and records every open — who, when, which screens, which demos played and for how long. Add a password for investors or anyone outside the deal.</p>
    <div class="grid3"><div class="field"><label>Recipient</label><input type="text" id="lk-recipient" placeholder="Jane Doe · Acme Capital"></div><div class="field"><label>Link name</label><input type="text" id="lk-name" placeholder="Investor deck · Series B"></div><div class="field"><label>Access</label><select id="lk-access" data-action-change="lk-access"><option value="token">Generated access token (recommended)</option><option value="password">Password I choose</option><option value="open">Open — anyone with the link</option></select></div></div>
    <div class="grid3" style="margin-top:8px"><div class="field" id="lk-pw-wrap" style="display:none"><label>Password</label><input type="text" id="lk-password" autocomplete="off" placeholder="what you will tell them"></div><div class="field" id="lk-access-hint" style="grid-column:span 2"><label>&nbsp;</label><div class="hint">A token like Q7KM-4XPT is generated and shown once. Send the link and the token separately, or use the one-click link.</div></div></div>
    <div style="margin-top:10px"><button class="btn primary" data-action="link-create">+ Create tracked link</button></div>
    ${S.issued ? `<div class="issued"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px"><b>🔑 ${esc(S.issued.name)}${S.issued.recipient ? ' · for ' + esc(S.issued.recipient) : ''}</b><button class="btn sm" data-action="link-issued-done">Done</button></div>
      <div class="irow"><span>Link</span><input type="text" readonly value="${esc(S.issued.shortUrl)}"><button class="btn sm" data-action="link-copy" data-url="${esc(S.issued.shortUrl)}">Copy</button></div>
      ${S.issued.token ? `<div class="irow"><span>Access token</span><input type="text" readonly class="tok" value="${esc(S.issued.token)}"><button class="btn sm" data-action="link-copy" data-url="${esc(S.issued.token)}">Copy</button></div>
      <div class="irow"><span>One-click link</span><input type="text" readonly value="${esc(S.issued.tokenUrl)}"><button class="btn sm" data-action="link-copy" data-url="${esc(S.issued.tokenUrl)}">Copy</button></div><div class="hint">The token is shown only now — it is stored hashed. “New token” in the table issues another (the old one stops working).</div>` : ''}</div>` : ''}</div>
...
function linksTableHtml() {
  ...
  return `<table class="lk"><thead><tr><th>Link</th><th>Opens at</th><th class="r">Views</th><th class="r">Viewers</th><th class="r">Plays</th><th class="r">Time</th><th>Last viewed</th><th></th></tr></thead><tbody>${rows.map(l => `
    <tr class="${l.disabled || l.expired ? 'off' : ''}"><td><b>${esc(l.name)}</b> ${l.access === 'token' ? '🔑' : l.protected ? '🔒' : ''}${l.disabled ? ' <span class="pill">OFF</span>' : ''}${l.expired ? ' <span class="pill">EXPIRED</span>' : ''}<div class="sub">${esc(l.recipient || '—')} · <a href="#" data-action="link-copy" data-url="${esc(S.shortBase + l.code)}">${esc(S.shortBase + l.code)}</a></div></td>
...
async function linkAction(a, b) {
  ...
  if (a === 'link-create') {
    const recipient = ($('#lk-recipient').value || '').trim(), name = ($('#lk-name').value || '').trim(), access = $('#lk-access').value, password = $('#lk-password').value || ''; const target = shareTargets().find(t => t.go === S.share.go);
    if (access === 'password' && !password) return toast('Type the password you will give them', true);
    const fullName = name || (recipient ? 'Deck for ' + recipient : target ? 'Deck · ' + target.label : 'Shared deck');
    try { const r = await api('/api/links', { method: 'POST', body: JSON.stringify({ name: fullName, recipient, access, password: access === 'password' ? password : '', params: shareParams() }) }); toast('Tracked link created'); S.issued = { name: fullName, recipient, shortUrl: r.shortUrl, token: r.token || null, tokenUrl: r.tokenUrl || null }; S.links = null; await loadLinks(); } catch (e) { toast(e.message, true); }
    return;
  }
  if (a === 'link-issued-done') { S.issued = null; renderSection(); return; }
  const code = b.dataset.code;
  if (a === 'link-rotate') { if (!confirm('Issue a new access token? The old token stops working immediately.')) return; try { const r = await api('/api/links/' + code, { method: 'PATCH', body: JSON.stringify({ rotateToken: true }) }); S.issued = { name: b.dataset.name, recipient: b.dataset.recipient, shortUrl: S.shortBase + code, token: r.token || null, tokenUrl: r.tokenUrl || null }; S.links = null; await loadLinks(); } catch (e) { toast(e.message, true); } return; }
...
app.addEventListener('change', e => { const t = e.target; if (t.id === 'lk-access') { const pw = $('#lk-pw-wrap'), hint = $('#lk-access-hint'); if (pw) pw.style.display = t.value === 'password' ? '' : 'none'; if (hint) { hint.style.display = t.value === 'password' ? 'none' : ''; hint.querySelector('.hint').textContent = t.value === 'token' ? 'A token like Q7KM-4XPT is generated and shown once. Send the link and the token separately, or use the one-click link.' : 'No gate. Views are still tracked.'; } return; } if (t.tagName === 'SELECT' && t.dataset.root === 'v') { set(S.v, t.dataset.path, t.value); S.vDirty = true; renderVStatus(); return; } if (t.tagName === 'SELECT' && t.dataset.root === 'share') { S.share[t.dataset.path] = t.value; const out = $('#share-url'); if (out) out.value = shareUrl(); return; } if (t.tagName === 'SELECT' && t.dataset.path) { let v = t.value; if (t.dataset.path.startsWith('ui.')) v = Number(v); set(S.cfg, t.dataset.path, v); markDirty(); } });
